...
`src/database.py`
Conecta a um projeto Postgres/Supabase (`DATABASE_URL_DEMO` ou `DATABASE_URL_PRODUCAO`, via connection pooler) e persiste os objetos validados. Diferente do antigo backend SQLite, o schema (`sql/schema_postgres.sql`) NAO e aplicado automaticamente por este modulo - como `CREATE POLICY` (RLS) nao e idempotente, o schema e aplicado manualmente, uma unica vez, no SQL Editor do Supabase; `inicializar_schema()` so confere que as tabelas esperadas existem, com um erro claro se alguma estiver faltando. As operacoes de escrita sao idempotentes por numero da OC e arquivo de origem, entao reprocessar o mesmo PDF nao duplica registros. Quando a mesma OC (mesmo numero + mesmo cliente) aparece em arquivos diferentes, provavel resultado do mesmo PDF sendo salvo duas vezes pela automacao de ingestao, o modulo sinaliza os registros com `status_extracao = 'possivel_duplicata'`, mas nunca os exclui - ver "Tratamento de duplicidades" abaixo. Tambem confere se a soma dos itens bate com o valor total declarado (`alerta_valor_divergente`), se a confianca relatada pelo modelo ficou abaixo do limite (`alerta_baixa_confianca`), e se o CNPJ do cliente/fornecedor passa na validacao de digito verificador (`alerta_cnpj_invalido`, via `src/validadores.py`). Backup do banco e responsabilidade do proprio Supabase (nao ha mais copia de arquivo local a fazer).
...
Os dois modos gravam em projetos Postgres/Supabase SEPARADOS (`DATABASE_URL_DEMO`, `DATABASE_URL_PRODUCAO` no `.env`), escolhidos automaticamente a partir de `--modo` em `pipeline.py`, ou sobrepostos manualmente via `--dsn`. `report_generator.py` segue a mesma logica com seu proprio `--modo`, gerando `relatorio_demo.html` ou `relatorio.html` a partir do banco correspondente. Essa separacao existe para que dado sintetico (usado para demonstrar o projeto) nunca apareca misturado com dado real de producao no mesmo painel ou na mesma consulta - ver [docs/arquitetura_webapp.md](arquitetura_webapp.md) para o passo a passo completo de configuracao do Supabase.
