...
Le o banco e gera um painel HTML autocontido, sem dependencia de CDN externo, no estilo cards. Estrutura pensada para leitura executiva de cima para baixo: indicadores e graficos de negocio primeiro, tabela de OCs recentes (com status OK/Revisar por linha) em seguida, uma central unica de alertas de qualidade de dado, e por fim uma secao de auditoria visualmente discreta. Nao inclui nenhum campo da tabela dados_clinicos.
